...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $result = mysqli_query($conn, $sql);
            $sql = "DELETE FROM itemvenda WHERE venda_id_venda = '$id'";
            $result = mysqli_query($conn, $sql);
            $msg = 'Você deletou a venda';
        } else {
...
<body class="listar">
    ...
    <main class="principal">
        <div class="conteudo">
        <div class="delete">
            <h2>Deletar venda #<?=$venda['id_venda']?></h2>
            <?php if ($msg): ?>
            <p><?=$msg?></p>
            <?php else: ?>
            <p>Você tem certeza que deseja deletar a venda #<?=$venda['id_venda']?>?</p>
            <div class="yesno">
                <a href="delete.php?id=<?=$venda['id_venda']?>&confirm=yes" class="verde">Sim</a>
                <a href="delete.php?id=<?=$venda['id_venda']?>&confirm=no" class="vermelho">Não</a>
            </div>
            <?php endif; ?>
        </div>
